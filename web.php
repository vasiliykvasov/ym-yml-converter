...
if (isset($_GET['file_feed']) && (!empty($_GET['file_feed']))) { // Если удаленный файл передан
    ...
    if (fopen($_GET['file_feed'], "r")) { // Если удаленный файл доступен для чтения
        ...
        http_response_code(400); // Вернуть код ответа 400 Плохой запрос
        exit; // Выйти, чтобы не было ответа 200 ОК
    }
}